...
import Animated, { Easing, FadeIn, useAnimatedProps, useSharedValue, withRepeat, withSpring, withTiming } from 'react-native-reanimated';
...
export default function App() {
  ...
  useEffect(() => {
    async function prepare() {
      try {
        ...
        // 3. Fake delay for Splash visibility (optional)
  ...
  const animatedWaveProps = useAnimatedProps(() => {
    ...
    return {
      d: `M ${-move} ${currentY} Q ${-move + width * 0.25} ${currentY - amplitude}, ${-move + width * 0.5} ${currentY} T ${-move + width} ${currentY} Q ${-move + width * 1.25} ${currentY - amplitude}, ${-move + width * 1.5} ${currentY} T ${-move + width * 2} ${currentY} V ${height} H ${-move} Z`,
    };
  });
      ...
      <View style={styles.splashContainer}>
        ...
        <Animated.View entering={FadeIn.duration(800)} style={styles.splashContent}>
          <View style={styles.logoBadge}>
            <Feather name="droplet" size={50} color="#0061A4" />
          </View>
          <Text style={styles.splashTitle}>Hydration Tracker</Text>
          <ActivityIndicator size="small" color="#0061A4" style={{ marginTop: 20 }} />
        </Animated.View>
      </View>
    );
  }
            ...
            history.map((item) => (
              <View key={item.id} style={styles.logItem}>
                <View>
                  ...
                  <Text style={styles.logDay}>{new Date(item.date).getDate() === new Date().getDate() ? 'Today' : 'Yesterday'}</Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  logoBadge: { width: 100, height: 100, borderRadius: 50, backgroundColor: '#D1E4FF', justifyContent: 'center', alignItems: 'center', marginBottom: 20 },
  splashTitle: { fontSize: 24, fontWeight: '800', color: '#1A1C1E' },
