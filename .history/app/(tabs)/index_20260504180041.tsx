...
// --- SUB-COMPONENT: WATER DROP FILL LOADER ---
function WaterLoader() {
  const fillProgress = useSharedValue(0);

  useEffect(() => {
    fillProgress.value = withTiming(1, { duration: 2500 });
  }, []);
    ...
    async function initialize() {
      try {
        ...
        await Promise.all([
          ...
          loadData(),
          // Ensure the loader stays visible for at least 2.5s for visual effect
            ...
            history.map((item) => (
              <View key={item.id} style={styles.logItem}>
                <View>
                  ...
                  <Text style={styles.logDay}>Today</Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  dropShape: {
    ...
    transform: [{ rotate: '-45deg' }], overflow: 'hidden', justifyContent: 'flex-end',
    borderWidth: 2, borderColor: '#0061A4',
  },
  dropFill: { width: '100%', backgroundColor: '#0061A4' },
  loaderText: { marginTop: 20, fontSize: 16, fontWeight: '600', color: '#0061A4' },
  // MAIN APP STYLES
